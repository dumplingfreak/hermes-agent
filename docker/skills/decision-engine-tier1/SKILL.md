...
5. Writes `~/vault/raw/finance/decision_engine/data_inputs/YYYY-MM-DD.md`
...
python3 "$HERMES_HOME/skills/decision-engine-tier1/tier1_collect.py" \
    --output-dir "$HERMES_HOME/vault/raw/finance/decision_engine/data_inputs"
```
...
## After collection
If `trigger_fired: true` → draft a decision card in `~/vault/raw/finance/decision_engine/decision_cards/`. Pull analyst views from `~/vault/wiki/finance/` (master_index → relevant trader/concept files) before interpreting. Do not invent prices — the data file is the only ground truth.
